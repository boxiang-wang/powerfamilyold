...
margin <- function(b0, betas, y, x, loss = c("hsvm", "power"), delta=2, qv=2) {
  loss <- match.arg(loss)
  nobs <- nrow(x)
...
KKTnp = function(b0, betas, y, x, lambda, lambda2, thr, 
               loss = c("hsvm", "power"), delta=2, qv=2) {
  loss = match.arg(loss)
  dl = margin(b0, betas, y, x, loss=loss, delta=delta, qv=qv)
  ctr = 0
  for (l in 1:length(lambda)) {
    ...
    for(j in 1:p)
    {
      if(betas[j,l]==0)
      {
        ...
        AA = dl[j,l] + lambda[l] * sign(betas[j,l]) + lambda2 * betas[j,l]
        if (abs(AA) >= thr)
        {
          ctr <- ctr + 1
        }
...
KKTp = function(b0, betas, y, x, lambda, lambda2, thr, 
                loss = c("hsvm", "power"), delta=2, qv=2) {
  loss = match.arg(loss)
  dl = margin(b0, betas, y, x, loss=loss, delta=delta, qv=qv)
  count0 = 0
...
KKT = function(b0, betas, y, x, lambda, lambda2, thr, 
               loss = c("hsvm", "power"), delta=2, qv=2, print.out = F)
{
  if(print.out == F)
  {
    KKTnp(b0=b0, betas=betas, y=y, x=x, lambda=lambda, 
                    lambda2=lambda2, thr=thr, 
                    loss = loss, delta=delta, qv=qv)
  } else
  {
    KKTp(b0=b0, betas=betas, y=y, x=x, lambda=lambda, 
                    lambda2=lambda2, thr=thr,  
...
KKTperctg = function(dat, lambda2, qv, eps, thr)
{
  ...
  KKT(m.temp$b0, m.temp$betas, dat$y, dat$x, m.temp$lambda, lambda2=lambda2, thr=thr, 
